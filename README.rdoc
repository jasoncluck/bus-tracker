...
4. rails server
  This starts the rails web server on localhost:3000.  You can now visit http://localhost:3000 and see
  the bus-tracker map application

4. script/daemon run poll_buses.rb
  This starts a daemon process that downloads updates of bus positions and inserts them into the database.
  This step is required to see buses move on the map.
