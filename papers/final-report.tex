...
Bus Tracker was implemented using the Ruby on Rails web framework, and the Google Maps Javascript API.  The Rails application interfaces with WMATA servers and maintains its own representation of the current state of the WMATA server in its databases.  It poll this periodically and updates its database accordingly.  
